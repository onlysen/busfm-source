...
function make_watermark($groundImage,$waterImage="",$waterPos=0,$waterText="",$fontfile="includes/arial.ttf",$xOffset=0,$yOffset=0) {
   $isWaterImage = FALSE;
	$fontSize=12;
     ...
     if($isWaterImage) { //图片水印
         ...
         imagettftext ( $ground_im, $fontSize, 0, $posX + $xOffset, $posY + $h + $yOffset, imagecolorallocate($ground_im,0,0,0), $fontfile, $waterText);
		 imagettftext ( $ground_im, $fontSize, 0, $posX + $xOffset+1, $posY + $h + $yOffset+1, imagecolorallocate($ground_im,255,255,255), $fontfile, $waterText);
     }
...
function format_datetime($time){
	...
	if($now<60){
		...
		return intval($now/60).'minute ago';
	}elseif($now<86400){
		return intval($now/3600).' hours ago';
	}else{
...
function read_static_cache($cache_name){
    ...
    $cache_file_path = ROOT_PATH . '/temps/static/' . $cache_name . '.php';
