...
function get_banner_name($banner_id){
	...
	$row=$GLOBALS['db']->getone("SELECT banner_name FROM ".$GLOBALS['db_prefix']."banner WHERE banner_id='$banner_id'");
	return $row[0];
}

function get_banner_image($banner_id){
	if(empty($banner_id))return'';
	$row=$GLOBALS['db']->getone("SELECT banner_image FROM ".$GLOBALS['db_prefix']."banner WHERE banner_id='$banner_id'");
	return $row[0];
}
...
function get_page_children($parent_id){
	$array=array();
	$res=$GLOBALS['db']->getall("SELECT * FROM ".$GLOBALS['db_prefix']."page WHERE parent_id=$parent_id ORDER BY page_id ASC");
	foreach($res as $row){
